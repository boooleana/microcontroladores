...
void Nokia5110_Init             (void); //Inicializaor da tela
void Nokia5110_OutChar          (char data); //Imprimir um char
void Nokia5110_OutString        (char *ptr); //Imprimir uma string
void Nokia5110_OutUDec          (uint16_t n); //Imprime decimal
void Nokia5110_OutDec           (uint16_t n); //Imprime decimal com sinal
void Nokia5110_SetCursor        (uint8_t newX, uint8_t newY); //Setar a posição na tela
void Nokia5110_Clear            (void); //Limpar a tela
void Nokia5110_DrawFullImage    (const uint8_t *ptr); //Background (bitmap da imagem)
...
void Nokia5110_MyDrawing(const uint8_t *ptr,uint8_t xpos, uint8_t ypos, uint8_t linhas, uint8_t colunas); //bitmap, posição x, posição y, nº de linhas que o vetor criou(altura/8),largura (programa te da)[fazer dependendo de algo];
...
void set_mapa_buffer            (const uint8_t *mapa);
